...
const gameInstructionsRaw = decodeHtmlEntities("Επέλεξε τις λέξεις δίπλα στις εικόνες.<br />Μόλις τελειώσεις πάτα \"Έλεγχος\".");
...
const masterItemsData: ItemData[] = [ /* ... your item data ... */
    { id: 0, imageKey: 'dog', imagePath: 'assets/images/name-the-picture/adorable-animal-breed-981062.jpg',  correctAnswer: decodeHtmlEntities("Σκύλος") },
    { id: 1, imageKey: 'car', imagePath: 'assets/images/name-the-picture/adventure-automobile-car-849835.jpg', correctAnswer: decodeHtmlEntities("Αμάξι") },
    { id: 2, imageKey: 'cat', imagePath: 'assets/images/name-the-picture/manja-vitolic-gKXKBY-C-Dk-unsplash.jpg', correctAnswer: decodeHtmlEntities("Γάτα") },
    { id: 3, imageKey: 'flag', imagePath: 'assets/images/name-the-picture/america-american-flag-blue-sky-951382.jpg', correctAnswer: decodeHtmlEntities("Σημαία") },
    { id: 4, imageKey: 'camera', imagePath: 'assets/images/name-the-picture/aperture-blur-bokeh-1561081.jpg', correctAnswer: decodeHtmlEntities("Φωτογραφική μηχανή") },
    { id: 5, imageKey: 'coffee', imagePath: 'assets/images/name-the-picture/art-blur-cappuccino-302899.jpg', correctAnswer: decodeHtmlEntities("Φλυτζάνι με καφέ") },
    { id: 6, imageKey: 'biscuits', imagePath: 'assets/images/name-the-picture/background-black-colors-239581.jpg', correctAnswer: decodeHtmlEntities("Μπισκότα") },
    { id: 7, imageKey: 'sandwich', imagePath: 'assets/images/name-the-picture/beef-blur-bread-1053769.jpg', correctAnswer: decodeHtmlEntities("Χάμπουγκερ-Σάντουιτς") },
    { id: 8, imageKey: 'watermelon', imagePath: 'assets/images/name-the-picture/close-up-delicious-fruit-1068534.jpg', correctAnswer: decodeHtmlEntities("Καρπούζι") },
    { id: 9, imageKey: 'motorbike', imagePath: 'assets/images/name-the-picture/bike-broken-city-1045535.jpg', correctAnswer: decodeHtmlEntities("Μηχανή"), imageHeight: 100 }
];
...
export class NameThePictureScene extends Phaser.Scene {
    ...
    init(data: { level?: string } = {}) {
        ...
        if (this.wordChoicePanel && this.wordChoicePanel.scene) this.wordChoicePanel.destroy();
    ...
    create() {
        ...
        let tempShuffledMasterList = Phaser.Utils.Array.Shuffle([...masterItemsData]);
        this.itemsToDisplayThisRound = tempShuffledMasterList.slice(0, ITEMS_PER_ROUND);

        const catItemIndex = this.itemsToDisplayThisRound.findIndex(item => item.imageKey === CAT_IMAGE_KEY);
        ...
        this.add.text(gameWidth / 2, headerCurrentY, gameTitle, {
            fontSize: `${titleFontSize}px`, color: '#000000', fontStyle: 'bold', align: 'center',
            wordWrap: { width: gameWidth - (headerContentPadding * 2) }
        }).setOrigin(0.5, 0);
        headerCurrentY += titleFontSize + 8;

        this.add.text(gameWidth / 2, headerCurrentY, gameSubtitle, {
            fontSize: `${subtitleFontSize}px`, color: '#111111', align: 'center',
            wordWrap: { width: gameWidth - (headerContentPadding * 2) }
        }).setOrigin(0.5, 0);
        headerCurrentY += subtitleFontSize + 12;

        const instructionsArray = gameInstructionsRaw.replace(/<br \/>/g, '\n').split('\n');
        instructionsArray.forEach((line) => {
            this.add.text(gameWidth / 2, headerCurrentY, line, {
                fontSize: `${instructionFontSize}px`, color: '#333333', align: 'center',
                wordWrap: { width: gameWidth - (headerContentPadding * 2) }
            }).setOrigin(0.5, 0);
        ...
        this.itemsToDisplayThisRound.forEach((itemData) => {
            ...
            let displayHeight = (tempImage.height / tempImage.width) * IMAGE_DISPLAY_WIDTH;
            if (itemData.imageHeight) displayHeight = itemData.imageHeight;
            preCalcItemsBlockHeight += (displayHeight + (IMAGE_BORDER_PADDING * 2) + imageBorderThickness * 2) + itemRowVerticalSpacing; 
        });
        ...
        const interactiveBlockStartY = headerCurrentY + Math.max(itemsColumnVerticalPadding, (remainingSpaceY - preCalcItemsBlockHeight) / 2) ;
        ...
        this.itemsToDisplayThisRound.forEach((itemData) => {
            ...
            const borderWidthWithPadding = IMAGE_DISPLAY_WIDTH + (IMAGE_BORDER_PADDING * 2);
            ...
            img.setDisplaySize(itemData.imageWidth || IMAGE_DISPLAY_WIDTH, displayHeight);
            rowContainer.add(img);
            
            const answerTextX = IMAGE_DISPLAY_WIDTH + IMAGE_BORDER_PADDING * 2 + elementSpacingInRow; 
            const answerText = this.add.text(answerTextX, 0, initialOptionText, {
                fontSize: '17px', color: '#0000FF', backgroundColor: '#EEEEEE',
                padding: { x: 10, y: 5 }, fixedWidth: answerTextWidth, align: 'center'
            }).setOrigin(0, 0.5).setInteractive({ useHandCursor: true });
            rowContainer.add(answerText);

            const currentItemElement: ItemElement = {
                id: itemData.id, imageSprite: img, answerTextSprite: answerText,
                feedbackTextSprite: null, rowContainer: rowContainer, imageBorderGfx: imageBorderGfx
            };
            ...
            answerText.on(Phaser.Input.Events.POINTER_DOWN, () => {
                if (this.gameEnded || (this.feedbackPopup && this.feedbackPopup.active)) return;
                this.activeItemElementForPanel = currentItemElement;
                const globalAnswerTextBounds = currentItemElement.answerTextSprite.getBounds();
                this.showWordChoicePanel(globalAnswerTextBounds.right + 10, globalAnswerTextBounds.top - 5);
            });
            
            const currentRowVisualHeight = borderHeightWithPadding + imageBorderThickness * 2; 
        ...
        const checkButtonX = gameWidth - checkButtonColumnMarginRight - (checkButtonWidth / 2); 
        const checkButtonY = interactiveBlockStartY + (actualItemsBlockHeight / 2);

        this.checkButton = this.add.text(checkButtonX, checkButtonY, checkButtonText, {
            fontSize: '20px', color: '#FFFFFF', backgroundColor: '#28a745', 
            padding: { x: 20, y: 10 }, fixedWidth: checkButtonWidth, align: 'center'
        }).setOrigin(0.5, 0.5); 
    ...
    showWordChoicePanel(x: number, y: number) {
        ...
        const panelHeight = allPossibleAnswers.length * itemHeight + (panelPadding * 2);
        ...
        if (panelX + panelWidth > this.cameras.main.width - panelPadding) {
            if (this.activeItemElementForPanel) {
                const bounds = this.activeItemElementForPanel.answerTextSprite.getBounds();
        ...
        allPossibleAnswers.forEach((word, index) => {
            const wordText = this.add.text(panelPadding, panelPadding + (index * itemHeight), word, {
                fontSize: '17px',
                color: '#212529', 
                padding: { top: (itemHeight - 17)/2 -1 , bottom:(itemHeight - 17)/2 +1,  x: 8},
                fixedWidth: panelWidth - (panelPadding * 2),
                align: 'left' 
            }).setInteractive({ useHandCursor: true });

            wordText.on(Phaser.Input.Events.POINTER_OVER, () => wordText.setBackgroundColor('#E9ECEF'));
            wordText.on(Phaser.Input.Events.POINTER_OUT, () => wordText.setBackgroundColor('rgba(0,0,0,0)')); 
    ...
    checkAllAnswers() {
        ...
        this.itemElements.forEach(element => {
            ...
            const feedbackIndicator = isCorrect ? correctIndicator : incorrectIndicator;
            const feedbackBorderColor = isCorrect ? 0x00CC00 : 0xFF0000; 
            
            element.imageBorderGfx.clear(); 
            element.imageBorderGfx.lineStyle(imageBorderThickness, feedbackBorderColor, 1);
            
            const tempImage = this.textures.get(element.imageSprite.texture.key).getSourceImage();
            let displayHeight = (tempImage.height / tempImage.width) * IMAGE_DISPLAY_WIDTH; 
            const itemDataForBorder = this.itemsToDisplayThisRound.find(it => it.id === element.id);
            if (itemDataForBorder && itemDataForBorder.imageHeight) displayHeight = itemDataForBorder.imageHeight;
            ...
            const answerTextRightEdge = element.answerTextSprite.x + element.answerTextSprite.width;
            element.feedbackTextSprite = this.add.text(
                answerTextRightEdge + 8, 
                element.answerTextSprite.y, 
                feedbackIndicator, {
                fontSize: '20px', color: isCorrect ? '#009900' : '#FF0000', fontStyle: 'bold'
            }).setOrigin(0, 0.5); 
        ...
        overallFeedbackMessage += (this.score === 100) ? correctResponseText : incorrectResponseText;
    ...
    showFeedbackPopup(message: string) {
        ...
        const gameWidth = this.cameras.main.width;
        const checkButtonTopY = this.checkButton.y - (this.checkButton.height * this.checkButton.originY); // Get precise top of check button
        ...
        bg.fillRoundedRect(-popupWidth / 2, -popupHeight / 2, popupWidth, popupHeight, 10);
        bg.lineStyle(2, 0xCCCCCC, 1); 
        bg.strokeRoundedRect(-popupWidth / 2, -popupHeight / 2, popupWidth, popupHeight, 10);
        this.feedbackPopup.add(bg);

        const feedbackText = this.add.text(0, -popupHeight / 2 + panelPadding + 5, message, {
            fontSize: '17px', color: '#333333', align: 'center',
        ...
        const playAgainButton = this.add.text(-popupWidth / 4, buttonsY, 'Παίξε Ξανά', {
            fontSize: '15px', color: '#FFFFFF', backgroundColor: '#17A2B8',
        ...
        const closeButton = this.add.text(popupWidth / 4, buttonsY, 'Κλείσιμο', { 
            fontSize: '15px', color: '#FFFFFF', backgroundColor: '#6C757D',
             padding: { x: 10, y: (buttonHeight - 15)/2 }, 
