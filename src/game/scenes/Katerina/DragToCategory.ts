...
create() {
  ...
  private setupLevel() {
    ...
    this.levelData.categories.forEach((category, index) => {
      ...
      const dropZone = this.add.zone(x, y, categoryWidth, categoryHeight)
        .setRectangleDropZone(categoryWidth, categoryHeight)
        .setData('category', category);

      const basketImage = this.add.image(x, y, 'basket')
        .setDisplaySize(categoryWidth, categoryHeight)
        .setDepth(1001);

      basketImage.setAlpha(0.85);

      this.add.text(x, y + categoryHeight / 3 , category, {
        fontSize: '30px',
        color: '#fff',
        fontFamily: 'Arial',
    ...
    const draggableItems = this.levelData.items.map(item => {
        const image = this.add.image(0, 0, item.name)
    ...
    const shelf2 = this.add.image((width / 2), shelfY2, 'shelf').setOrigin(0.5, 0.5);
    ...
    draggableItems.forEach(item => {
      ...
      item.on('pointerover', () => {
        ...
        hoverTimer = item.scene.time.delayedCall(2000, () => {
          ...
          tooltipText = item.scene.add.text(item.x, item.y - 20, `Κατηγορία: ${category}`, {
            fontSize: '18px',
            color: '#ffffff',
  ...
  private setupAssistAnimation(draggableItems: Phaser.GameObjects.Image[]) {
    ...
    const assistItem = draggableItems.find(item => item.getData('category') === this.levelData.categories[0]);
  ...
  private createSoundToggle() {
    ...
    const soundIcon = this.add.image(80, 50, 'soundOn')
  ...
  private createHelpIcon() {
    const helpIcon = this.add.text(this.scale.width - 30, 50, '?', {
        fontSize: '32px',
        color: '#000',
        backgroundColor: '#fff',
        padding: { x: 10, y: 5 }
  ...
  private showHelpPanel() {
      ...
      panel.fillRoundedRect(panelX - panelWidth / 2 + 4, panelY - panelHeight / 2 + 4, panelWidth, panelHeight, 20);
      ...
      closeButton.on('pointerover', () => closeButton.setStyle({ backgroundColor: '#ff4444' }));
      closeButton.on('pointerout', () => closeButton.setStyle({ backgroundColor: '#cc3333' }));
...
private onLevelComplete() {
  ...
  const text = this.add.text(width / 2, height / 2, 'Το ολοκλήρωσες με επιτυχία.\nΣυγχαρητήρια!', {
    fontSize: '26px',
    fontFamily: 'Arial',
    color: '#ffffff',
    align: 'center',
  ...
  private createExitButton() {
    const exitButton = this.add.text(this.scale.width - 80, 50, 'Έξοδος', {
      fontSize: '24px',
      color: '#ffffff',
    ...
    exitButton.on('pointerover', () => exitButton.setStyle({ backgroundColor: '#ff3333' }));
    exitButton.on('pointerout', () => exitButton.setStyle({ backgroundColor: '#cc0000' }));
