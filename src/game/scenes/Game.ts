...
export class Game extends Scene {
	private games: GameInfo[] = [
		{
			key: "DragToCategory",
			name: "Βρες την Κατηγορία",
			levels: ["level-01", "level-02", "level-03"],
		},
		{
			key: "FindThePairs",
			name: "Βρες τα Ζευγάρια",
			levels: ["Easy", "Medium", "Hard", "Expert"],
		},
		{
			key: "NameThePictureScene",
			name: "Ονόμασε την Εικόνα",
			levels: [],
		},
	];
	...
	create() {
		...
		this.titleText = this.add
			.text(centerX, 100, "Επίλεξε Παιχνίδι", {
				fontFamily: "Segoe UI",
				fontSize: 40,
				color: "#4a3c31",
				stroke: "#9c8b70",
	...
	private showLevelsMenu(game: GameInfo) {
		...
		if (game.levels.length === 0) this.scene.start(game.key, {});
